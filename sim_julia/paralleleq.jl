...
function main()
    ...
    b = .5 #BENEFIT FROM AN INTERACTION
    c = 1.0 #COST TO THE ALTRUIST IN THE INTERACTION
    delta = 0.1 #SELECTION STRENGTH
    ...
    m_c = m_critico(1,b,c,n,delta,beta)
    ...
    arq6 = open("pterminou", "w")
    arq7 = open("pnaoterminou", "w")
...
function Simulate10(N,n,A,LA,b,c,beta,delta,PM,PMIG,op)
    ...
    @everywhere num_its = Int32[]

    @everywhere res = (Bool, Int32)[]

    @parallel for i in 1:11
        push!(res, Simulate(N,n,A,LA,b,c,beta,delta,PM,PMIG,op))
        push!(num_its, res[length(res)][2])
...
# Run one simulation
@everywhere function Simulate(N,n,A,LA,b,c,beta,delta,PM,PMIG,op)
...
@everywhere function Regression(N,n,A,LA,b,c,beta,delta,PM)
...
@everywhere function Initialize(n_groups, n_ind, n_A, DEMES)
...
@everywhere function Make_contest(DEMES, N, n, LA, b, c, beta, delta)
...
@everywhere function Insert_mutation(n_groups, n_ind, mu, DEMES)
...
@everywhere function Make_migration(N, n, PMIG, DEMES)
    #MIGRATION
    VDEME = Int32[]
    VPOS = Int32[]
    VMIG = Int32[]
...
@everywhere function Save_fig(N, n, A, IC, AL, b, beta, PMIG)
...
@everywhere function Save_file(x, y, filename)
    arq = open(filename,"w")
...
function m_critico(w0,b,c,n,delta,beta)
    ...
    return so.brentq(rho, 0., 1., xtol=0.001, args=(w0,b,c,n,delta,beta))
end

# Funcao rho: calcula (autovalor dominante - 1) a partir dos parametros
function rho(m,w0,b,c,n,delta,beta)
...
function sel(k,l,n,delta,b,c,w0,beta)
    w = fitness(0,k,n,delta,b,c,w0)
    p = k*w/(n*fitness_m(k,n,delta,b,c,w0))
    
    d = Binomial(n,p)
    
    return (1-beta+2*beta*pVencer(k,n))*pdf(d,l)
end
    
    
# Matriz de selecao - recebe n e parametros delta,b,c,w0 e beta
function matriz_selecao(n,delta,b,c,w0,beta)

    #S = np.fromfunction((i,j)-> sel(i,j,n,delta,b,c,w0,beta),(n+1,n+1))
    
    S = Array(Float64, n+1,n+1)
    
    for k in 1:n+1
        for l in 1:n+1
            S[(n+1)*(l-1)+k] = sel(k-1,l-1,n,delta,b,c,w0,beta)
        end
    end
