...
def testa(b, vc):
    nome_arq = "testeTLFW_mc_b=%.2f.txt" %b
    f = open(nome_arq, "w")
    precisao = 0.001
...
def main():
    ...
    vcost = [0.03, 0.15, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 5.0, 10.]
    # testar com 0., 1., 2., 10.
    for benefit in [0.,1.,2.,10.]:
        logger.info(u"Parâmetros fixos nessa simulação: N=%d, n=%d, b=%.2f, \
             delta=%.3f, \n\t\tmu=%.4f" \
                %(N, n, benefit, delta, mu))
